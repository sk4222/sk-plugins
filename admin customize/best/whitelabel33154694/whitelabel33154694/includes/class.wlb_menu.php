...
class wlb_menu {
	...
	function get_id_from_menu($str,$prefix='m_'){
		...
		if(!$wlb_plugin->is_wlb_administrator()){
			if(isset($menu)&&is_array($menu)&&count($menu)>0){
				foreach($menu as $k => $m){
					...
					$id = $this->get_id_from_menu($m[2],'m_');
					if(html_entity_decode($m[2])==$wlb_plugin->get_option($id)){
						unset($menu[$k]);
					} 
				}			
			}
			if(isset($submenu)&&is_array($submenu)&&count($submenu)>0){
				foreach($submenu as $key => $submenu_group){
					foreach($submenu_group as $k => $m){
						...
						$id = $this->get_id_from_menu($m[2],'sm_');
						if(html_entity_decode($m[2])==$wlb_plugin->get_option($id)){
							unset($submenu[$key][$k]);
						} 
					}					
		...
		if(is_array($menu)&&count($menu)>0){
			foreach($menu as $k => $m){
				...
				$t[$i]->options[] = (object)array(
					...
					'option_value'=> html_entity_decode($m[2]),
					'el_properties' => array(),
					'save_option'=>true,
					'load_option'=>true
				);					
